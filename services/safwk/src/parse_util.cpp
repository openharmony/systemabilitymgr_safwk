...
void ParseUtil::OpenSo(SaProfile& saProfile)
{
    if (saProfile.handle == nullptr) {
        ...
        HILOGI(TAG, "[PerformanceTest] SA:%{public}d OpenSo spend %{public}" PRId64 " ms",
        ...
        if (handle == nullptr) {
            ...
            HILOGE(TAG, "dlopen %{public}s failed with errno:%s!", fileName.c_str(), dlerror());
            return;
        }
...
{
    HILOGI(TAG, "config path:%{private}s", profilePath.c_str());
    if (!CheckPathExist(profilePath.c_str())) {
        HILOGE(TAG, "bad profile path!");
        return false;
    }
    std::unique_ptr<xmlDoc, decltype(&xmlFreeDoc)> docPtr(
        xmlReadFile(profilePath.c_str(), nullptr, XML_PARSE_NOBLANKS), xmlFreeDoc);
    if (docPtr == nullptr) {
        HILOGE(TAG, "ParseTrustConfig xmlReadFile error!");
        return false;
    }
    xmlNodePtr rootNodePtr = xmlDocGetRootElement(docPtr.get());
    if (!CheckRootTag(rootNodePtr)) {
        HILOGW(TAG, "ParseTrustConfig wrong root element tag!");
        return false;
    }
    ...
    for (; currNodePtr != nullptr; currNodePtr = currNodePtr->next) {
        ...
        if (nodeName == XML_TAG_NAME && processName.empty()) {
            ...
            if (!ParseProcess(currNodePtr, processName)) {
                HILOGE(TAG, "ParseTrustConfigInner wrong name tag!");
                return false;
            }
            ...
            if (!ParseSaId(currNodePtr, saId)) {
                HILOGE(TAG, "ParseTrustConfigInner wrong said tag!");
                continue;
            }
