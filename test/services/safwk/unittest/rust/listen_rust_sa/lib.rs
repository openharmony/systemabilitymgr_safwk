...
/// Function code of IListen
pub enum IListenCode {
    /// echo_str code
...
/// Function between proxy and stub of IListenService
pub trait IListen: IRemoteBroker {
    /// Test echo_str 
...
fn on_remote_request(stub: &dyn IListen, code: u32, data: &BorrowedMsgParcel,
...
define_remote_object!(
    IListen["ohos.ipc.test"] {
        stub: TestStub(on_remote_request),
        proxy: TestProxy,
    }
);

// Make RemoteStub<TestStub> object can call IListen function directly.
impl IListen for RemoteStub<TestStub> {
    fn echo_str(&self, value: &str) -> IpcResult<String> {
...
impl IListen for TestProxy {
    fn echo_str(&self, value: &str) -> IpcResult<String> {
        info!(LOG_LABEL,"TestProxy echo_str");
        let mut data = MsgParcel::new().expect("MsgParcel should success");
        data.write(value)?;
        let reply =
            self.remote.send_request(IListenCode::CodeEchoStr as u32, &data, false)?;
    ...
    fn request_concurent(&self, is_async: bool) -> IpcResult<bool> {
        ...
        let reply =
            self.remote.send_request(IListenCode::CodeRequestConcurrent as u32, &data, is_async)?;
